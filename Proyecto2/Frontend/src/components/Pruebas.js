...
function Pruebas() {
  ...
  //------ PARA DASHBOARD--------
  const [humedad, setHumedad] = useState([]) //Biogas
  const [turbidezIn, setTurbidezIn] = useState([])// Temperatura
  const [nivelagua, setNivelagua] = useState([]) // Chispa
  const [turbidez, setTurbidez] = useState([]) // Gas
  ...
  function fillDasboard(data) {
    setHumedad(info => data[data.length - 1].BioGas)
    setTurbidezIn(info => data[data.length - 1].Temperatura)
    setNivelagua(info => data[data.length - 1].Estado)
  ...
  useEffect(() => {
    ...
    /* ***********GRAFICA2 -> HUMEDAD*********************/
    const Array = datos.map(({ Fecha, NivelAgua, BioGas, Turbidez,TurbidezIn, PH }, i) => {
      var fecha_string = new Date(Fecha);
      if(selectedValued!=null){
        if(fecha_string.getDate() + '/' + (fecha_string.getMonth() + 1)==selectedValued.label){
          wa.push(Math.trunc(100-((BioGas*100)/1024)))
      ...
      return(
        Math.trunc(100-((BioGas*100)/1024))
      )
    ...
    const turb1 = datos.map(({ Fecha, NivelAgua, Humedad, Turbidez,Temperatura, PH }, i) => {
      var fecha_string = new Date(Fecha);
      if(selectedValued!=null){
        if(fecha_string.getDate() + '/' + (fecha_string.getMonth() + 1)==selectedValued.label){
          tu1.push(Temperatura)
          console.log(selectedValued)
    ...
    const niv = datos.map(({ Fecha, BioGas, Humedad, Turbidez,TurbidezIn, PH }, i) => {
      var fecha_string = new Date(Fecha);
      if(selectedValued!=null){
        if(fecha_string.getDate() + '/' + (fecha_string.getMonth() + 1)==selectedValued.label){
          nivA.push(((BioGas*452)/660))
          console.log(selectedValued)
    ...
    const turb2 = datos.map(({ Fecha, NivelAgua, Humedad, estado,TurbidezIn, PH }, i) => {
      var fecha_string = new Date(Fecha);
      if(selectedValued!=null){
        if(fecha_string.getDate() + '/' + (fecha_string.getMonth() + 1)==selectedValued.label){
          tu2.push(estado)
    ...
    <div>
      ...
      <section className='cards-wrapper2'>
        <div className="card-grid-space">
          ...
          <a className="cardY" >
            <div><h2>Estados gen chispa</h2></div>
            <CuadroX>
              <h3>0 (Apagado)</h3>
              <h3>1 (Encendido)</h3>
          ...
          <a className="card">
            <Line
              data={{
                ...
                datasets: [
                  {
                    label: "cant de metano vs tiempo",
                    data: graph2,
                    borderColor: "orange",
                    borderWidth: 1,
                  },
          ...
          <a className="card" >
            <Line
              data={{
                ...
                datasets: [
                  {
                    label: "Cant. de metano vs temperatura",
                    data: graph3,
                    borderColor: "red",
                    borderWidth: 1,
